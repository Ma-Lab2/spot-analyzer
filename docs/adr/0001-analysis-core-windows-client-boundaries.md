...
> **后继工作流说明（2026-09-13）**：上述单图和用户确认区域是本 ADR 的历史产品范围，不再约束 `workflow-contract-v1` 的客户端交互。Issue #75/ADR-0002 可以提供自动预览、多个独立工作项和最终一次确认；本 ADR 的 WPF 外壳、独立 Python 核心/worker、版本化 NDJSON、显示与测量分离以及不可变分析记录边界仍然有效。旧记录继续按生成时的契约和版本解释。
