...
public sealed class WorkerClient
{
    ...
    public Task<WorkerOutcome> RunPngAsync(string path, string sha256, bool semanticsConfirmed, CancellationToken cancellationToken, TimeSpan? timeout = null) =>
        RunAsync(new { kind = "png", path, sha256, intensity_semantics_confirmed = semanticsConfirmed, uri_hint = path }, cancellationToken, timeout);
